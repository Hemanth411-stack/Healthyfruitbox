...
const ProductImage = React.memo(({ productId, imageUrl, alt }) => {
  const [src, setSrc] = useState(imageUrl);
  const [loaded, setLoaded] = useState(false);

  return (
    <div className={`relative w-full h-40 bg-gray-100 rounded-md overflow-hidden transition-opacity duration-300 {loaded ? 'opacity-100' : 'opacity-0'}`}>
      <img
...
const CartPage = () => {
  ...
  const handleQuantityChange = (productId, productType, newQuantity) => {
    if (newQuantity > 0) {
  ...
  const handleRemoveItem = (productId, productType) => {
      ...
      <div className="flex items-center mb-8">
        ...
        {memoizedItems.length > 0 && (
          <span className="ml-4 bg-gray-100 text-gray-800 text-sm font-medium px-2.5 py-0.5 rounded">
            {memoizedItems.length} {memoizedItems.length === 1 ? 'Item' : 'Items'}
          </span>
        )}
      </div>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex justify-between items-center">
                <p className="text-sm text-gray-500">
                  Subtotal ({memoizedItems.length} {memoizedItems.length === 1 ? 'item' : 'items'})
                </p>
                <p className="text-lg font-semibold text-gray-900">
                  {(totalPrice)}
                </p>
